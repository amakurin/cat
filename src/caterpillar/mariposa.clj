...
;; (re-seq
;; #"(?iux)
;;    (?<= ([89][\doOоОзЗбБЧч_[^\p{IsCyrillic}\w]]{2,20}))
;;    [oOоОзЗбБЧч_[^\p{IsCyrillic}\d\-\w\s\(\)\:\.\,]]
;;    (?=([\doOоОзЗбБЧч_[^\p{IsCyrillic}\w]]{5,20}))
;;  #  \b"

;;  " 18 м. кв., к спальне присоединена лоджия в виде небольшого кабинета. Гостиная объединена с кухней и большой квадратной лоджией, где располагается просторная столовая. В квартире имеется современная новая мебель: спальный гарнитур, состоящий из двуспальной кровати, большого платяного шкафа и комода, встроенная кухня, диван, столовая группа, состоящая из раскладывающегося обеденного стола и мягкого уголка. Кроме того в квартире имеется вся необходимая бытовая техника: холодильник стиральная машина автомат Бош, микроволновая печь, электроплита с керамической поверхностью,
;;  электрическая духовка, телевизор. 50% по факту заселения Teл 92,12,2014, 12:23"
;;  )
...
(defn solve-formula [rules formula]
  (let [entity (-> formula :v last :v)
    ...
    (cond (= :rule-for-last (first rule))
          (solve-formula rules (assoc formula :v [(-> formula :v last)]))
          (and (= :not (first rule)) (or (true? entity-val)(false? entity-val)))
